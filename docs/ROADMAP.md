...
- [ ] **Multi-format (XLSX / Calc):** _Phase A + B done_ — open/new/save **XLSX** (Calc), document-kind detection (`src/engine/formats.ts`, model `supportsService`), kind-aware native + PDF export filters, `newDocument(kind)` / `getFile()` across engine · embed bridge · SDK · React; **native Calc chrome hidden** (own toolbar only — `hideAllToolbars` now covers `CalcWindowState`, sidebar hidden per-kind; formula bar + row/col headers + sheet tabs kept as the functional view); **type-aware toolbar** (`onKind` → `.calc-mode`; Writer-only controls `.dxe-writer-only` hidden in Calc); **cell-targeted `insertText`** + multi-sheet **`mergeFields`** for Calc. All verified live (open/save round-trip, kind detection, cell merge). _Remaining_ — interactive Find/Replace for Calc (Writer-only `XSearchable`; the Find button is hidden in Calc for now), Calc-specific tools (number format, borders, merge cells, insert/delete rows/cols), and formula-bar refresh after programmatic edits.
