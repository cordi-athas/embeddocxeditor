...
- [ ] **Multi-format (XLSX / Calc):** _Phase A + B done_ — open/new/save **XLSX** (Calc), document-kind detection (`src/engine/formats.ts`, model `supportsService`), kind-aware native + PDF export filters, `newDocument(kind)` / `getFile()` across engine · embed bridge · SDK · React; **native Calc chrome hidden** (own toolbar only — `hideAllToolbars` now covers `CalcWindowState`, sidebar hidden per-kind; formula bar + row/col headers + sheet tabs kept as the functional view); **type-aware toolbar** (`onKind` → `.calc-mode`; Writer-only controls `.dxe-writer-only` hidden in Calc); **cell-targeted `insertText`** + multi-sheet **`mergeFields`** for Calc. **Find/Replace for Calc** (active-sheet find, all-sheets replace-all); **Calc-specific tools** in the type-aware toolbar — a number-format select (General/Percent/Currency/Date) and a cells select (insert/delete row·column, merge). All verified live (open/save round-trip, kind detection, cell merge, Calc find/replace, type-aware toolbar toggle). _Remaining_ — cell borders, numeric `setValue` for programmatic number entry (insertText stores text in Calc), multi-sheet find navigation, formula-bar refresh after programmatic edits.
