...
- [x] **Multi-format (XLSX / Calc):** _Phase A + B + gap-closing done_ — open/new/save **XLSX** (Calc), document-kind detection (`src/engine/formats.ts`, model `supportsService`), kind-aware native + PDF export filters, `newDocument(kind)` / `getFile()` across engine · embed bridge · SDK · React; **native Calc chrome hidden** (own toolbar only — `hideAllToolbars` now covers `CalcWindowState`, sidebar hidden per-kind; formula bar + row/col headers + sheet tabs kept as the functional view); **type-aware toolbar** (`onKind` → `.calc-mode`; Writer-only controls `.dxe-writer-only` hidden in Calc); **cell-targeted `insertText`** with **numeric `setValue`** (numeric-looking input is stored as a real number so number formats apply, not text) + multi-sheet **`mergeFields`** for Calc; **formula-bar refresh** after programmatic edits (`refreshCalcView` re-selects the active selection). **Find/Replace for Calc** — multi-sheet find that walks every sheet from the active one and switches the active sheet on a hit, plus all-sheets replace-all. **Calc-specific tools** in the type-aware toolbar — a number-format select (General/Percent/Currency/Date) and a cells select (insert/delete row·column, merge). All verified live (open/save round-trip, kind detection, `1234.5` → `$1,234.50` currency with a numeric formula-bar value, multi-sheet find/not-found, cell merge, type-aware toolbar toggle).
  - **Known limitation — Calc cell borders (not implemented):** borders require setting the `TableBorder`/`TableBorder2` (or per-edge `*Border`) cell properties, whose line structs all carry a `com.sun.star.util.Color` typedef field. This LibreOffice-WASM / zetajs build cannot marshal that typedef in **either** direction — constructing a `BorderLine`/`BorderLine2` throws `bad type description com.sun.star.util.Color`, and even **reading** an existing `TableBorder2` back throws the same (verified across four approaches: construct v2, construct-without-Color, get-mutate-set v2, construct v1). There is no parameterless border dispatch to fall back on (`.uno:SetBorderStyle` needs a struct arg). The non-functional borders control was removed rather than shipped broken; revisit if a future zetajs build gains `util::Color` support.
